...
| `rescheduling.py`          | 실제 재배치 알고리즘(Rescheduling Algorithm) 이 구현된 파일입니다. 다음 5가지 알고리즘을 지원:
• spread – 파드를 가장 적게 가진 노드로 분산 배치
• binpack – CPU 사용률이 높은 노드로 집중 배치
• random – 임의의 정상 노드로 재배치
• kubescheduling – 쿠버네티스 기본 스케줄러 정책 사용
• communication – CAR(Communication-Aware Rescheduling): 파드 간 통신 관계를 고려하여, 상호 통신이 많은 파드들을 같은 노드에 재배치하고 통신 비용을 최소화                     |
